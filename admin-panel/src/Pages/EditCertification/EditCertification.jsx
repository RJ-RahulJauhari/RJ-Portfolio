...
const EditCertification = () => {
  ...
  const setForm = async () => {
    try {
      await axios.get(`${BASE_URL}/certifications/get/${id}`)
        .then((res) =>{
          const data = res.data;
          console.log(data);
          if(data){
            ...
            setCredId(data.credId);
            setCredUrl(data.credUrl);
  ...
  const uploadCertificate = () => {
    if(newCertificate){
      const certificateDestination = `certification/${user._id}/${id}/certificate/${new Date().getTime().toString()+certificate.name}`;
  ...
  const updateCertification = async () => {
    ...
    try {
      await axios.patch(`${BASE_URL}/certifications/update/${id}`,obj,{withCredentials:true})
        .then((res) =>{
          const data = res.data;
          if(data){
            console.log(data)
  ...
  return (
    <div className='add-certification'>
        <h1 className='title'>Edit Certification</h1>
    ...
    <div className='bottom-buttons'>
        ...
        <button onClick={() => {updateCertification()}} className='submit-btn'>Update</button>
    </div>
