...
const EditExperience = () => {
  ...
  const setForm = async () => {
    try {
      axios.get(`${BASE_URL}/experience/get/${id}`)
      .then(async (res) => {
        const data = res.data;
        if(data){
  ...
  // Update Experience Functionality 
  ...
  const updateExperience = async () => {
    ...
    try {
      await axios.patch(`${BASE_URL}/experience/update/${id}`,obj,{withCredentials:true})
        .then((res) => {
          const data = res.data;
          if(data){
            console.log(data)
  ...
  return (
      ...
      <div className='bottom-buttons'>
        ...
        <button onClick={() => {updateExperience()}} className='submit-btn'>Update Experience</button>
      </div>
    </div>
  )
}
